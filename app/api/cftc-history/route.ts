import { assets, type CftcSnapshot, type TraderRow } from "../../cftc-data";
...
function traderRow(row: RawRow, fields: { name: string; long: string; short: string; spread?: string; longChange: string; shortChange: string }): TraderRow {
  const longChange = numeric(row, fields.longChange);
  const shortChange = numeric(row, fields.shortChange);
  return {
    name: fields.name,
    long: numeric(row, fields.long),
    short: numeric(row, fields.short),
    spreading: fields.spread ? numeric(row, fields.spread) : undefined,
    longChange,
    shortChange,
...
export async function GET(request: Request) {
  ...
  const requestedLimit = Number(searchParams.get("limit") ?? 52);
  ...
  try {
    ...
    const snapshots = rows
      .map(asset.reportType === "Disaggregated" ? transformDisaggregated : transformTff)
      .filter((snapshot) => snapshot.date);

    return Response.json(
      { symbol: asset.symbol, source: endpoint.origin, snapshots },
