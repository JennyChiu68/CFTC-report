...
type DetailTab = "positions" | "history" | "depth";
...
function DeepPremiumView({ assetList, onAsset }: { assetList: CftcAsset[]; onAsset: (asset: CftcAsset) => void }) {
  const [selectedSymbol, setSelectedSymbol] = useState("XAU"); const selected = assetList.find((asset) => asset.symbol === selectedSymbol) ?? assetList[0];
  return <div className="deep-premium-view"><p className="premium-selector-label">选择品种查看深度解读</p><div className="premium-selector">{assetList.map((asset) => <button type="button" key={asset.symbol} className={selectedSymbol === asset.symbol ? "active" : ""} onClick={() => { setSelectedSymbol(asset.symbol); trackEvent("pro_asset", { symbol: asset.symbol }); }}>{screenName(asset)}</button>)}</div><button type="button" className="full-data-link" onClick={() => onAsset(selected)}><span>查看 {screenName(selected)} 完整持仓数据</span><b>›</b></button><PremiumAnalysis asset={selected} /></div>;
}

function DepthPanel({ asset, premium, onUnlock }: { asset: CftcAsset; premium: boolean; onUnlock: () => void }) { return <div className="tab-panel">{premium ? <PremiumAnalysis asset={asset} /> : <PremiumGate preview="detail" onUnlock={onUnlock} />}</div>; }
...
function DetailView({ asset, tab, setTab, onBack, premium, onUnlock }: { asset: CftcAsset; tab: DetailTab; setTab: (tab: DetailTab) => void; onBack: () => void; premium: boolean; onUnlock: () => void }) {
  ...
  return <div className="cot-scroll detail-scroll"><section className="detail-titlebar"><button className="back" onClick={onBack} aria-label="返回">‹</button><div><h1>{screenName(asset)} <span>{screenSymbol(asset)}</span></h1><div className="date-control"><button className="date-button" type="button" aria-expanded={dateMenuOpen} aria-haspopup="listbox" onClick={() => setDateMenuOpen((open) => !open)}>{displayReportDate(selectedDate)} <small>{selectedDate === latestDate ? "最新" : "历史"}</small><b>{dateMenuOpen ? "⌃" : "⌄"}</b></button>{dateMenuOpen && <div className="date-menu" role="listbox" aria-label="选择CFTC报告日期">{historyLoading && <div className="date-menu-status">正在加载CFTC历史数据…</div>}{historyError && !historyLoading && <div className="date-menu-status error">历史数据加载失败<button type="button" onClick={() => { setHistoryError(false); setHistoryLoading(true); setHistoryRequest((value) => value + 1); }}>重试</button></div>}{!historyLoading && !historyError && dateOptions.map((snapshot, index) => <button key={snapshot.date} type="button" role="option" aria-selected={snapshot.date === selectedDate} className={snapshot.date === selectedDate ? "active" : ""} onClick={() => { setSelectedDate(snapshot.date); setDateMenuOpen(false); trackEvent("report_date", { symbol: asset.symbol, date: snapshot.date }); }}><span>{displayReportDate(snapshot.date)}</span>{index === 0 && <small>最新</small>}</button>)}{!historyLoading && !historyError && dateOptions.length > 0 && <p>CFTC官方 · 可选最近52期；每期保留后续52周趋势</p>}</div>}</div></div><button className="refresh" aria-label="刷新" onClick={() => { setHistoryError(false); setHistoryLoading(true); setHistoryRequest((value) => value + 1); }}>↻</button></section>
    ...
    {tab === "positions" && <PositionsPanel asset={viewAsset} totalChange={totalChange} />}{tab === "history" && <HistoryPanel asset={viewAsset} />}{tab === "depth" && <DepthPanel asset={viewAsset} premium={premium} onUnlock={onUnlock} />}
...
export default function Home() {
  ...
  return <main className="cot-app"><BrandHeader premium={premium} />{screen === "home" && <HomeView filter={filter} setFilter={setFilter} onOpen={openAsset} assetList={assetList} reportDate={reportDate} syncedAt={market?.syncedAt} loading={marketLoading} unavailable={market?.unavailable ?? []} />}{screen === "detail" && <DetailView key={selected.symbol} asset={selected} tab={tab} setTab={setTab} onBack={() => setScreen("home")} premium={premium} onUnlock={unlockPremium} />}{screen === "pro" && <ProView premium={premium} onUnlock={unlockPremium} onAsset={(asset) => openAsset(asset, "depth")} assetList={assetList} reportDate={reportDate} />}<BottomNav screen={screen} onHome={() => setScreen("home")} onPro={() => { setScreen("pro"); trackEvent("open_pro"); }} /></main>;
}
